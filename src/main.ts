...
// 绘制不同的形状+画笔+清除按钮
const toolBtns = document.querySelectorAll(".tool") as unknown as HTMLLIElement[];
// 是否以填充形式绘制
const fillColor = document.getElementById("fill-color") as HTMLInputElement;
// 线条的尺寸
const sizeSlider = document.getElementById("size-slider") as HTMLInputElement;
// 颜色选择按钮
const colorBtns = document.querySelectorAll(".colors .option") as unknown as HTMLLIElement[];
// 颜色选择器按钮
...
// 绘制时线宽度由range输入框决定
sizeSlider.addEventListener("change", function changeBrushWidth() {
    // brushWidth = (e.target as HTMLInputElement).value;
...
toolBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
        (document.querySelector(".options .active") as HTMLLIElement).classList.remove("active");
...
colorBtns.forEach(btn => {
    btn.addEventListener("click", (e) => {
        ...
        // 给当前按钮添加样式
        (e.target as HTMLLIElement).classList.add("selected");
        // 从window上获取选中对象的颜色
        selectedColor = (e.target as HTMLLIElement).style.backgroundColor;
    });
});
...
colorPicker.addEventListener("change", (e) => {
    ...
    colorPicker.parentElement!.style.backgroundColor = (<HTMLInputElement>e.target).value;
